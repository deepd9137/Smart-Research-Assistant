...
cp .env.example .env
# Edit .env: GOOGLE_API_KEY=... TAVILY_API_KEY=... PINECONE_API_KEY=...
streamlit run streamlit_app.py
```

Open the URL shown in the terminal, upload up to **5 PDFs**, click **Build knowledge base**, then chat.

---

## Deployment guide

### Streamlit Community Cloud (recommended for this repo)

1. Push the project to GitHub (include `requirements.txt`, `streamlit_app.py`, `app/`).  
2. Go to [share.streamlit.io](https://share.streamlit.io) and **New app** → select the repo.  
...
5. Deploy. The app runs server-side; uploaded PDFs are processed in memory / temp, and embeddings are persisted in Pinecone.

**Note:** Pinecone access needs outbound internet and a valid API key. Keep keys in Streamlit secrets, never in git.

### Hugging Face Spaces

1. Create a **Streamlit** Space.  
2. Copy this project structure; set **Secrets** same as above.  
3. Entry command typically uses `streamlit run streamlit_app.py` per Space template.

### Vercel

Vercel is not ideal for long-running Streamlit servers; prefer Streamlit Cloud or HF Spaces. If you need Vercel, you’d typically expose a **FastAPI** backend and a separate frontend — out of scope for this template.
...
**[0:00–0:30] Intro**  
“Hi, I’m showing a Smart Research Assistant built for PDF research. It uses RAG with Gemini, Pinecone for vectors, and Tavily when the documents don’t contain the answer.”

**[0:30–1:30] Demo flow**  
“Here I upload a few PDFs — at most five — and click Build knowledge base. The app extracts text, chunks it, embeds with Gemini, and stores vectors in Pinecone. I’ll ask a question that’s clearly in the paper: you’ll see an answer with bracket citations like one and two, and references listing file name and page. I’ll ask a follow-up to show conversation memory.”

**[1:30–2:30] Fallback**  
“Next I’ll ask something obscure that isn’t in the PDFs. Retrieval scores look weak, so the app routes to Tavily, and the answer cites web results as W1, W2. I’ll mention you should still verify critical facts from the sources.”

**[2:30–4:00] Architecture**  
“Architecture in one minute: PyPDFLoader gives us page-level metadata so citations are accurate. Recursive splitting with overlap balances context and precision. We retrieve top five from Pinecone by similarity; if the best match is too weak, we don’t trust the corpus and we hit the web. Confidence in the UI is a heuristic — useful for demos, not a formal probability.”

**[4:00–4:45] Decisions & wrap**  
“I chose Pinecone to match take-home requirements and keep retrieval production-style. LangChain keeps the pipeline modular. Future work: reranking, hybrid search, and stronger evaluation. Thanks for watching.”

---

## License

Use freely for learning and interviews; add your own license if you ship publicly.
